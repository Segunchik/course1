...
def test_load_user_settings():
    ...
    with tempfile.NamedTemporaryFile(delete=False, mode="w+", encoding="utf-8") as temp_file:
        ...
        test_data = {"user_currencies": ["USD", "EUR", "GBP"], "user_stocks": ["AAPL", "GOOG", "TSLA", "AMZN"]}
        ...
        assert (
            result == test_data
        ), f"Ошибка при чтении существующего файла. Ожидалось: {test_data}, получено: {result}"
...
def test_get_expenses_by_card(sample_filtered_df):
    ...
    for res, exp in zip(result, expected_output):
        assert res["card_num"] == exp["card_num"], "Карточный номер не совпадает"
        assert res["total_expenses"] == exp["total_expenses"], "Расходы не совпадают"
        assert res["cashback"] == exp["cashback"], "Кэшбэк не совпадает"
...
def test_missing_columns():
    missing_column_df = pd.DataFrame({"Номер карты": [], "Сумма операции": [], "Кэшбэк": []}).astype(
        {"Номер карты": "str"}
    )
...
def test_get_top5_transaction_empty_dataframe():
    ...
    empty_df = pd.DataFrame(
        columns=["Дата платежа", "Сумма операции", "Сумма операции с округлением", "Категория", "Описание"]
    )
...
def test_get_currency_rate_success(mock_load_user_settings, mock_requests_get):  # , mock_load_user_settings):
    ...
    expected_result = [{"currency": "USD", "rate": 73.54}, {"currency": "EUR", "rate": 85.21}]
...
MOCK_USER_SETTINGS_STOCKS = {"user_stocks": ["AAPL", "GOOGL", "MSFT"]}
...
def mock_yf_tickers():
    with patch("src.utils.yf.Tickers") as mock_tickers:
        ...
        mock_tickers_dict = {
            symbol: MagicMock(info=MOCK_STOCK_DATA[symbol]) for symbol in MOCK_USER_SETTINGS_STOCKS["user_stocks"]
        }

        # Настраиваем поведение mock_tickers
        mock_tickers.return_value = MagicMock(tickers=mock_tickers_dict)
        yield mock_tickers
